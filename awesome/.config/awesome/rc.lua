	...
	awful.spawn.easy_async_with_shell("bluetoothctl show | grep Powered | awk '{print $2}'", function(is_bluetooth_on)
		if string.match(is_bluetooth_on, "yes") then
			awful.spawn.easy_async_with_shell(
				"bluetoothctl info >> /dev/null && echo connected  || echo on ",
...
local network = wibox.widget({
	...
	buttons = gears.table.join(awful.button({}, 1, function()
		awful.spawn.with_shell(
			"net nmtui" .. "; echo 'sb_ethernet()' | awesome-client; echo 'sb_wifi()' | awesome-client"
		)
		sb_ethernet()
		sb_wifi()
	end)),
...
--           {description = "go back", group = "tag"}),
	...
	awful.key({ modkey }, "t", function()
		awful.spawn(terminal .. " -e tmux new-session -A -s main")
	end, { description = "open a terminal with tmux", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "t", function()
		awful.spawn("rofi-tmux")
	...
	awful.key({ modkey }, "Print", function()
		awful.spawn("copy-selected-area")
	end, { description = "copy screen", group = "launcher" }),
	awful.key({ modkey }, "XF86Calculator", function()
		awful.spawn("speedcrunch")
	end, { description = "copy screen", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "Print", function()
		awful.spawn("maimpick-rofi")
	end, { description = "copy screen selection", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "w", function()
		awful.spawn(terminal .. " -e nmtui")
	end, { description = "nmtui", group = "launcher" }),
	...
	awful.key({ modkey, "Control" }, "n", function()
		awful.spawn("mpc seek +00:00:05")
	end, { description = "play song +5sec", group = "launcher" }),
	...
	end, { description = "search for books", group = "launcher" }),
	awful.key({ modkey }, "i", function()
		awful.spawn("rofi-innop")
	end, { description = "search in innop folder", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "r", function()
		awful.spawn(terminal .. " -e htop")
	end, { description = "htop", group = "launcher" }),
	awful.key({ modkey }, "p", function()
		awful.spawn('rofi -terminal alacritty -show run -icon-theme "Papirus" -show-icons')
	end, { description = "program launcher", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "p", function()
		awful.spawn('rofi -terminal alacritty -show emoji -icon-theme "Papirus" -show-icons')
	...
	awful.key({ modkey }, "F12", function()
		awful.spawn("showcam")
	end, { description = "show web cam if any", group = "launcher" }),
	...
	awful.key({ modkey, "Control" }, "=", function()
		...
		sb_orange()
	end, { description = "increase light temperature", group = "launcher" }),
	...
	awful.key({ modkey, "Control" }, "b", function()
		awful.spawn("blueberry")
	end, { description = "blueberry", group = "launcher" }),
	awful.key({ modkey, "Control" }, "m", function()
		awful.spawn(terminal .. " -e pulsemixer")
	end, { description = "pulsemixer", group = "launcher" }),
...
tag.connect_signal("request::screen", function(t)
	for s in screen do
		if
			s ~= t.screen
			and s.geometry.x == t.screen.geometry.x
			and s.geometry.y == t.screen.geometry.y
			and s.geometry.width == t.screen.geometry.width
			and s.geometry.height == t.screen.geometry.height
		then
